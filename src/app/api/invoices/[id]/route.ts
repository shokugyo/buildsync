import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '@/lib/auth'
import { prisma } from '@/lib/prisma'
import { hasRole, APPROVER_ROLES } from '@/lib/permissions'

export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  const session = await getServerSession(authOptions)
  if (!session) return NextResponse.json({ error: '認証が必要です' }, { status: 401 })

  const invoice = await prisma.invoice.findFirst({
    where: { id: params.id, companyId: (session.user as any).companyId },
    include: {
      project: { select: { id: true, name: true, projectNumber: true, address: true } },
      customer: { select: { id: true, name: true } },
      items: { orderBy: { sortOrder: 'asc' } },
    },
  })

  if (!invoice) return NextResponse.json({ error: '請求書が見つかりません' }, { status: 404 })
  return NextResponse.json(invoice)
}

export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  const session = await getServerSession(authOptions)
  if (!session) return NextResponse.json({ error: '認証が必要です' }, { status: 401 })

  const body = await req.json()
  const { amount, customerId, invoiceDate, dueDate, status, notes, items, paidDate } = body

  if (status === '承認済' && !hasRole(session, APPROVER_ROLES)) {
    return NextResponse.json({ error: '承認権限が必要です' }, { status: 403 })
  }

  try {
    let finalAmount = amount !== undefined ? Number(amount) : undefined
    if (items !== undefined && items.length > 0) {
      finalAmount = items.reduce((s: number, it: any) => s + (parseFloat(it.amount) || 0), 0)
    }

    // Auto-set paidDate when transitioning to 入金済
    let autoPaidDate: Date | undefined
    if (status === '入金済' && paidDate === undefined) {
      const current = await prisma.invoice.findUnique({ where: { id: params.id }, select: { paidDate: true } })
      if (!current?.paidDate) {
        autoPaidDate = new Date()
      }
    }

    const invoice = await prisma.invoice.update({
      where: { id: params.id },
      data: {
        ...(finalAmount !== undefined && {
          amount: finalAmount,
          taxAmount: Math.round(finalAmount * 0.1),
          totalAmount: finalAmount + Math.round(finalAmount * 0.1),
        }),
        ...(customerId !== undefined && { customerId: customerId || null }),
        ...(invoiceDate !== undefined && { invoiceDate: invoiceDate ? new Date(invoiceDate) : null }),
        ...(dueDate !== undefined && { dueDate: dueDate ? new Date(dueDate) : null }),
        ...(status !== undefined && { status }),
        ...(paidDate !== undefined && { paidDate: paidDate ? new Date(paidDate) : null }),
        ...(autoPaidDate !== undefined && { paidDate: autoPaidDate }),
        ...(notes !== undefined && { notes: notes || null }),
        ...(items !== undefined && {
          items: {
            deleteMany: {},
            create: items.map((it: any, idx: number) => ({
              name: it.name,
              quantity: parseFloat(it.quantity) || 1,
              unitPrice: parseFloat(it.unitPrice) || 0,
              amount: parseFloat(it.amount) || 0,
              sortOrder: idx,
              taxRate: it.taxRate !== undefined ? parseFloat(it.taxRate) : 10,
            })),
          },
        }),
      },
      include: {
        project: { select: { id: true, name: true, projectNumber: true } },
        customer: { select: { id: true, name: true } },
        items: { orderBy: { sortOrder: 'asc' } },
      },
    })
    return NextResponse.json(invoice)
  } catch {
    return NextResponse.json({ error: '更新に失敗しました' }, { status: 500 })
  }
}

export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  const session = await getServerSession(authOptions)
  if (!session) return NextResponse.json({ error: '認証が必要です' }, { status: 401 })

  try {
    await prisma.invoice.delete({ where: { id: params.id } })
    return NextResponse.json({ success: true })
  } catch {
    return NextResponse.json({ error: '削除に失敗しました' }, { status: 500 })
  }
}
