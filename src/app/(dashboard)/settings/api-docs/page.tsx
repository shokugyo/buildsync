...
const API_GROUPS: ApiGroup[] = [
  ...
  {
    tag: '公開API (v1)',
    description: 'APIキー認証による外部連携API',
    endpoints: [
      { method: 'GET', path: '/api/v1/projects', summary: '案件一覧（外部API）', description: 'X-API-Key ヘッダーで認証。ページネーション対応。', params: [{ name: 'page', type: 'number', required: false, description: 'ページ番号（デフォルト: 1）' }, { name: 'limit', type: 'number', required: false, description: '件数（最大100）' }], response: '{ data: Project[], total: number, page: number }' },
      { method: 'GET', path: '/api/v1/tasks', summary: '工程一覧（外部API）', params: [{ name: 'projectId', type: 'string', required: false, description: '案件ID' }], response: 'Schedule[]' },
      { method: 'POST', path: '/api/v1/tasks', summary: '工程作成（外部API）', response: 'Schedule' },
    ],
  },
  {
    tag: 'エクスポート',
