...
export default function ContractsAnalyticsPage() {
  ...
  const contracted = projects.filter(p => p.contractAmount && p.contractAmount > 0)
  const noContractDate = projects.filter(p => !p.startDate)

  // Group by 案件種別 for distribution
  ...
  const allTypes = Array.from(new Set(contracted.map((p: any) => p.propertyType || 'その他')))
    ...
    <div>
      ...
      <div className="min-h-screen bg-slate-50">
        <div className="bg-white border-b border-slate-200 px-6 py-3">
          <h2 className="text-base font-semibold text-slate-900">契約推移</h2>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 mb-3 flex flex-wrap gap-2 items-center">
            <div className="flex items-center gap-1 mr-2">
              <span className="text-xs text-slate-500">契約日</span>
          ...
          <div className="grid grid-cols-3 gap-4 mb-5">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
              <p className="text-3xl font-bold text-blue-600">{contracted.length}</p>
              <p className="text-xs text-slate-400 mt-1">契約数 <InfoIcon /></p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
              <p className="text-3xl font-bold text-red-500">{noContractDate.length}</p>
              <p className="text-xs text-slate-400 mt-1">契約日未入力案件数 <InfoIcon /></p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h3 className="text-sm font-semibold text-slate-700 mb-3">契約分布 <InfoIcon /></h3>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h3 className="text-sm font-semibold text-slate-700 mb-3">契約推移 <InfoIcon /></h3>
              {trendData.length === 0 ? (
                <div className="h-48 flex flex-col items-center justify-center text-slate-300">
                  <Info className="w-8 h-8 mb-2" />
                  <p className="text-sm">該当なし</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-3">
              <h3 className="text-sm font-semibold text-slate-700">契約推移表 <InfoIcon /></h3>
              <div className="flex gap-1 ml-auto">
                <button className="px-2 py-0.5 bg-blue-600 text-white text-xs rounded">案件種別</button>
                <button className="px-2 py-0.5 border border-slate-300 text-slate-600 text-xs rounded hover:bg-slate-50">契約数</button>
              </div>
            </div>
            {contracted.length === 0 ? (
                  ...
                  <tbody>
                    {allTypes.map(t => (
                      <tr key={t} className="border-t border-slate-100">
                        <td className="px-3 py-2 text-slate-700">{t}</td>
                        {trendData.map(d => <td key={d.month} className="px-3 py-2 text-right">{(d as any)[t] || 0}</td>)}
                        <td className="px-3 py-2 text-right font-semibold">{contracted.filter(p => (p.propertyType || 'その他') === t).length}</td>
                      </tr>
                    ))}
                    <tr className="border-t border-slate-200 bg-slate-50">
                      <td className="px-3 py-2 font-semibold text-slate-700">合計</td>
                      {trendData.map(d => <td key={d.month} className="px-3 py-2 text-right font-semibold">{Object.entries(d).filter(([k]) => k !== 'month').reduce((sum, [, v]) => sum + (Number(v) || 0), 0)}</td>)}
                      <td className="px-3 py-2 text-right font-semibold">{contracted.length}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
