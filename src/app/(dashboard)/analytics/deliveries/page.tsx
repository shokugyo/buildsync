...
export default function DeliveriesAnalyticsPage() {
  ...
  const withDelivery = projects.filter(p => p.deliveryDate)
  const noDeliveryDate = projects.filter(p => !p.deliveryDate)
  ...
  const allTypes = Array.from(new Set(withDelivery.map((p: any) => p.propertyType || 'その他')))
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-5">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
              <p className="text-3xl font-bold text-blue-600">{withDelivery.length}</p>
              <p className="text-xs text-slate-400 mt-1">引渡数 <InfoIcon /></p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
              <p className="text-3xl font-bold text-red-500">{noDeliveryDate.length}</p>
              <p className="text-xs text-slate-400 mt-1">引渡日未入力案件数 <InfoIcon /></p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h3 className="text-sm font-semibold text-slate-700 mb-3">引渡分布 <InfoIcon /></h3>
              {distributionData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={Math.max(160, distributionData.length * 60)}>
                  <BarChart data={distributionData} layout="vertical">
                    ...
                    <XAxis type="number" tick={{ fontSize: 10 }} allowDecimals={false} label={{ value: '引渡数', position: 'insideBottom', offset: -2, fontSize: 10 }} />
                    <YAxis type="category" dataKey="name" tick={{ fontSize: 11 }} width={70} />
                    <Tooltip />
                    <Bar dataKey="value" name="引渡数" fill="#3b82f6">
                      {distributionData.map((entry, i) => <rect key={i} fill={getColor(entry.name, i)} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              )}
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h3 className="text-sm font-semibold text-slate-700 mb-3">引渡推移 <InfoIcon /></h3>
              {trendData.length === 0 ? (
                <div className="h-48 flex flex-col items-center justify-center text-slate-300">
                  <Info className="w-8 h-8 mb-2" /><p className="text-sm">該当なし</p>
                </div>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={trendData}>
                    ...
                    {allTypes.map((t, i) => <Bar key={t} dataKey={t} stackId="a" fill={getColor(t, i)} name={t} />)}
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-3">
              <h3 className="text-sm font-semibold text-slate-700">引渡推移表 <InfoIcon /></h3>
              <div className="flex gap-1 ml-auto">
                <button className="px-2 py-0.5 bg-blue-600 text-white text-xs rounded">案件種別</button>
                <button className="px-2 py-0.5 border border-slate-300 text-slate-600 text-xs rounded hover:bg-slate-50">引渡数</button>
              </div>
            </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {allTypes.map((t, ti) => (
                      <tr key={t} className="border-t border-slate-100">
                        <td className="px-3 py-2 flex items-center gap-1.5">
                          <span className="w-3 h-3 rounded-sm inline-block" style={{ background: getColor(t, ti) }} />{t}
                        </td>
                        {trendData.map(d => <td key={d.month} className="px-3 py-2 text-right">{(d as any)[t] || 0}</td>)}
                        <td className="px-3 py-2 text-right font-semibold">{withDelivery.filter(p => (p.propertyType || 'その他') === t).length}</td>
                      </tr>
                    ))}
                    <tr className="border-t border-slate-200 bg-slate-50">
                      ...
                      <td className="px-3 py-2 text-right font-semibold">{withDelivery.length}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
