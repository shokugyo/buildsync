...
import { BarChart2, Plus, Edit2, Trash2, X, TrendingUp, Download, Printer, AlertTriangle, ChevronUp, ChevronDown } from 'lucide-react'
import {
...
interface EVMMetrics {
  ...
  projectName: string
  projectNumber: string
  bac: number        // Budget at Completion（契約金額）
  pv: number         // Planned Value（予算金額合計）
  ev: number         // Earned Value（発注金額 × 進捗率≒発注金額）
  ac: number         // Actual Cost（請求金額合計）
  sv: number         // Schedule Variance = EV - PV
  cv: number         // Cost Variance = EV - AC
  spi: number        // Schedule Performance Index = EV / PV
  cpi: number        // Cost Performance Index = EV / AC
  eac: number        // Estimate at Completion = AC + (BAC - EV) / CPI
  variance: number   // 差異 = pv - ac
  isOverBudget: boolean
}

type SortKey = keyof Pick<EVMMetrics, 'projectName' | 'bac' | 'pv' | 'ac' | 'cv' | 'cpi' | 'eac'> | 'consumptionRate'
type SortDir = 'asc' | 'desc'

const COLORS = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899']

const TABS = ['原価一覧', 'EVM分析', '粗利サマリー'] as const
...
export default function CostsPage() {
  ...
  const [projects, setProjects] = useState<{ id: string; name: string; projectNumber: string; contractAmount?: number | null }[]>([])
  const [showModal, setShowModal] = useState(false)
  const [editTarget, setEditTarget] = useState<Budget | null>(null)
  const [form, setForm] = useState(defaultForm)
  const [saving, setSaving] = useState(false)
  const [trendData, setTrendData] = useState<TrendDataPoint[]>([])
  const [activeTab, setActiveTab] = useState<Tab>('原価一覧')
  const [evmSort, setEvmSort] = useState<{ key: SortKey; dir: SortDir }>({ key: 'projectName', dir: 'asc' })
  ...
  const evmMetrics: EVMMetrics[] = (() => {
    ...
    return projectIds.map((pid) => {
      const rows = budgets.filter((b) => b.project?.id === pid)
      const proj = rows[0]?.project
      // プロジェクト情報はAPIから取得済みのprojectsリストも参照してcontractAmountを確定
      const apiProject = projects.find((p) => p.id === pid)
      const bac = (apiProject?.contractAmount ?? proj?.contractAmount ?? 0) as number
      const pv = rows.reduce((s, b) => s + b.budgetAmount, 0)
      // EV = 発注金額（進捗率の代替として発注済を出来高とみなす）
      const ev = rows.reduce((s, b) => s + b.orderedAmount, 0)
      const ac = rows.reduce((s, b) => s + b.invoicedAmount, 0)
      const sv = ev - pv
      const cv = ev - ac
      const spi = pv > 0 ? ev / pv : 0
      const cpi = ac > 0 ? ev / ac : 0
      // EAC = AC + (BAC - EV) / CPI; CPIが0の場合はBAC
      const eac = cpi > 0 ? ac + (bac - ev) / cpi : bac
      const variance = pv - ac
      return {
        projectId: pid,
        projectName: proj?.name ?? '',
        projectNumber: proj?.projectNumber ?? '',
  ...
  const overBudgetCount = evmMetrics.filter((m) => m.isOverBudget).length

  // EVMサマリー合計
  const evmTotal = evmMetrics.reduce(
    (acc, m) => ({
      bac: acc.bac + m.bac,
      pv: acc.pv + m.pv,
      ev: acc.ev + m.ev,
  ...
  const evmTotalSV = evmTotal.ev - evmTotal.pv
  const evmTotalCV = evmTotal.ev - evmTotal.ac
  const evmTotalCPI = evmTotal.ac > 0 ? evmTotal.ev / evmTotal.ac : 0
  const evmTotalSPI = evmTotal.pv > 0 ? evmTotal.ev / evmTotal.pv : 0

  // EVMソート
  const sortedEvm = [...evmMetrics].sort((a, b) => {
    let av: number
    let bv: number
    if (evmSort.key === 'consumptionRate') {
      av = a.pv > 0 ? (a.ac / a.pv) * 100 : 0
      bv = b.pv > 0 ? (b.ac / b.pv) * 100 : 0
    } else if (evmSort.key === 'projectName') {
      return evmSort.dir === 'asc'
        ? a.projectName.localeCompare(b.projectName, 'ja')
        : b.projectName.localeCompare(a.projectName, 'ja')
    } else {
      av = a[evmSort.key] as number
      bv = b[evmSort.key] as number
    }
    return evmSort.dir === 'asc' ? av - bv : bv - av
  })
  ...
  const SortIcon = ({ col }: { col: SortKey }) => {
    if (evmSort.key !== col) return <ChevronUp className="w-3 h-3 text-slate-300 inline ml-0.5" />
    return evmSort.dir === 'asc'
      ...
      <div className="p-6">
          ...
          <>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 mb-6">
                  ...
                  <svg viewBox={`0 0 ${chartW} ${chartH}`} width="100%" className="overflow-visible">
                    ...
                    {trendData.map((d, i) => {
                      ...
                      return (
                        <g key={d.label}>
                          <rect x={cx - barW - 2} y={bY} width={barW} height={bH} fill="#3b82f6" rx="2" opacity="0.85" />
                          <rect x={cx + 2} y={aY} width={barW} height={aH} fill="#f97316" rx="2" opacity="0.85" />
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <a
                  href="/api/export/costs?format=csv"
                  download
                ...
                <a
                  href="/api/export/costs?format=excel"
                  download
                  className="flex items-center gap-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 px-3 py-2 rounded-lg text-sm font-medium border border-emerald-200"
                >
                  <Download className="w-4 h-4" />
                  Excel出力
                </a>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
                      ...
                      {pieData.length > 0 ? (
                        ...
                        <div className="h-64 flex items-center justify-center text-slate-400">データなし</div>
                      )}
                    </div>
                  </div>
                )}
        ...
        {activeTab === 'EVM分析' && (
          <>
            {/* アラートバナー */}
            {overBudgetCount > 0 && (
              <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-3 mb-6 text-sm font-medium">
                <AlertTriangle className="w-4 h-4 flex-shrink-0" />
                <span>⚠ {overBudgetCount}件の案件で予算超過が発生しています</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <p className="text-xs text-slate-500 mb-1">BAC（総予算）</p>
                <p className="text-lg font-bold text-slate-900">{formatCurrency(evmTotal.bac)}</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <p className="text-xs text-slate-500 mb-1">PV（計画値）</p>
                <p className="text-lg font-bold text-blue-600">{formatCurrency(evmTotal.pv)}</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <p className="text-xs text-slate-500 mb-1">EV（出来高）</p>
                <p className="text-lg font-bold text-indigo-600">{formatCurrency(evmTotal.ev)}</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <p className="text-xs text-slate-500 mb-1">AC（実コスト）</p>
                <p className="text-lg font-bold text-orange-600">{formatCurrency(evmTotal.ac)}</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <p className="text-xs text-slate-500 mb-1">SV（工程差異）</p>
                <p className={`text-lg font-bold ${evmTotalSV < 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {formatCurrency(evmTotalSV)}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <p className="text-xs text-slate-500 mb-1">CV（コスト差異）</p>
                <p className={`text-lg font-bold ${evmTotalCV < 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {formatCurrency(evmTotalCV)}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <p className="text-xs text-slate-500 mb-1">CPI（コスト効率）</p>
                <p className={`text-lg font-bold ${evmTotalCPI < 1 ? 'text-red-600' : evmTotalCPI > 1.2 ? 'text-green-600' : 'text-slate-900'}`}>
                  {evmTotalCPI.toFixed(2)}
                </p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                <p className="text-xs text-slate-500 mb-1">SPI（工程効率）</p>
                <p className={`text-lg font-bold ${evmTotalSPI < 1 ? 'text-red-600' : evmTotalSPI > 1.2 ? 'text-green-600' : 'text-slate-900'}`}>
                  {evmTotalSPI.toFixed(2)}
                </p>
              </div>
            </div>
            ...
            {/* SVGバーチャート（差分グラフ） */}
            {evmMetrics.length > 0 && (() => {
              const allVals = evmMetrics.flatMap((m) => [m.pv, m.ev, m.ac])
              ...
              return (
                <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5 mb-6">
                  <h2 className="font-semibold text-slate-900 mb-4">案件別 EVM 差分グラフ（PV・EV・AC）</h2>
                  <svg viewBox={`0 0 ${chartW} ${chartH}`} width="100%" className="overflow-visible">
                    {/* グリッド & Y軸 */}
                    {Array.from({ length: yTicks + 1 }).map((_, i) => {
                      const val = (maxVal / yTicks) * i
                    ...
                    {evmMetrics.map((m, i) => {
                      ...
                      const evY = toY(m.ev)
                      const acY = toY(m.ac)
                      const pvH = Math.max(0, padT + innerH - pvY)
                      const evH = Math.max(0, padT + innerH - evY)
                      const acH = Math.max(0, padT + innerH - acY)
                      const acColor = m.isOverBudget ? '#ef4444' : '#10b981'
                      // 案件名を折り返し（最大10文字）
                      const label = m.projectName.length > 8 ? m.projectName.slice(0, 7) + '…' : m.projectName
                      return (
                        <g key={m.projectId}>
                          {/* PV bar (blue) */}
                          <rect x={cx - barW * 1.5 - 2} y={pvY} width={barW} height={pvH} fill="#3b82f6" rx="2" opacity="0.85" />
                          {/* EV bar (orange) */}
                          <rect x={cx - barW * 0.5} y={evY} width={barW} height={evH} fill="#f97316" rx="2" opacity="0.85" />
                          {/* AC bar (green or red) */}
                          <rect x={cx + barW * 0.5 + 2} y={acY} width={barW} height={acH} fill={acColor} rx="2" opacity="0.85" />
                          {/* X label */}
                          <text x={cx} y={padT + innerH + 16} textAnchor="middle" fontSize="10" fill="#64748b">{label}</text>
                        </g>
                      )
                    })}
                    ...
                    {/* 凡例 */}
                    <rect x={padL} y={chartH - 18} width={10} height={10} fill="#3b82f6" rx="2" />
                    <text x={padL + 14} y={chartH - 9} fontSize="11" fill="#475569">PV（計画値）</text>
                    <rect x={padL + 100} y={chartH - 18} width={10} height={10} fill="#f97316" rx="2" />
                    <text x={padL + 114} y={chartH - 9} fontSize="11" fill="#475569">EV（出来高）</text>
                    <rect x={padL + 200} y={chartH - 18} width={10} height={10} fill="#10b981" rx="2" />
                    <text x={padL + 214} y={chartH - 9} fontSize="11" fill="#475569">AC（実コスト）</text>
                    <rect x={padL + 310} y={chartH - 18} width={10} height={10} fill="#ef4444" rx="2" />
                    <text x={padL + 324} y={chartH - 9} fontSize="11" fill="#475569">AC（超過）</text>
                  </svg>
                </div>
              )
            })()}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-100">
                ...
                <h2 className="font-semibold text-slate-900">案件別 EVM 詳細</h2>
              </div>
              {loading ? (
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50">
                      <tr>
                        {(
                          [
                            { key: 'projectName' as SortKey, label: '案件名', align: 'left' },
                            { key: 'bac' as SortKey, label: 'BAC（契約）', align: 'right' },
                            { key: 'pv' as SortKey, label: 'PV（計画）', align: 'right' },
                            { key: 'ac' as SortKey, label: 'AC（実コスト）', align: 'right' },
                            { key: 'cv' as SortKey, label: 'CV（差異）', align: 'right' },
                            { key: 'cpi' as SortKey, label: 'CPI', align: 'right' },
                            { key: 'eac' as SortKey, label: 'EAC（完工予測）', align: 'right' },
                            { key: 'consumptionRate' as SortKey, label: '予算消化率', align: 'right' },
                          ] as { key: SortKey; label: string; align: string }[]
                        ).map((col) => (
                          <th
                            key={col.key}
                            className={`px-4 py-3 text-xs font-medium text-slate-500 uppercase cursor-pointer select-none hover:bg-slate-100 transition-colors ${col.align === 'right' ? 'text-right' : 'text-left'}`}
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {sortedEvm.map((m) => {
                        const consumptionRate = m.pv > 0 ? (m.ac / m.pv) * 100 : 0
                        const rowBg =
                          m.cpi < 1.0
                            ...
                            : 'hover:bg-slate-50'
                        return (
                          <tr key={m.projectId} className={`transition-colors ${rowBg}`}>
                            <td className="px-4 py-3 font-medium text-slate-900">
                              <span className="text-xs text-slate-400 font-mono mr-1">{m.projectNumber}</span>
                              {m.projectName}
                            </td>
                            <td className="px-4 py-3 text-right text-slate-900">{formatCurrency(m.bac)}</td>
                            <td className="px-4 py-3 text-right text-slate-900">{formatCurrency(m.pv)}</td>
                            <td className="px-4 py-3 text-right text-slate-900">{formatCurrency(m.ac)}</td>
                            <td className={`px-4 py-3 text-right font-medium ${m.cv < 0 ? 'text-red-600' : 'text-green-600'}`}>
                              {formatCurrency(m.cv)}
                            </td>
                            <td className={`px-4 py-3 text-right font-bold ${m.cpi < 1.0 ? 'text-red-600' : m.cpi > 1.2 ? 'text-green-600' : 'text-slate-900'}`}>
                              {m.cpi.toFixed(2)}
                            </td>
                            <td className="px-4 py-3 text-right text-slate-900">{formatCurrency(m.eac)}</td>
                            <td className="px-4 py-3 text-right">
                              <span className={`font-medium ${consumptionRate > 100 ? 'text-red-600' : consumptionRate > 90 ? 'text-orange-500' : 'text-emerald-600'}`}>
                                {consumptionRate.toFixed(1)}%
                              </span>
                              <div className="w-full bg-slate-100 rounded-full h-1.5 mt-1">
                                <div
                                  className={`h-1.5 rounded-full ${consumptionRate > 100 ? 'bg-red-500' : consumptionRate > 90 ? 'bg-orange-400' : 'bg-green-400'}`}
