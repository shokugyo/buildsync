...
interface ProjectSummary {
  ...
  openDefects: number
  recentPhotos: { id: string; filePath: string; caption?: string | null; takenAt?: string | null }[]
}
...
export default function CustomerPortalPage() {
  ...
  const [activeTab, setActiveTab] = useState<'projects' | 'photos' | 'inquiry'>('projects')
  ...
  const handleSendInquiry = async () => {
    ...
    try {
      const res = await fetch('/api/customer-portal/inquiry', {
        ...
        body: JSON.stringify({ projectId: selectedProjectId || null, content: inquiryText }),
      })
      if (res.ok) {
        setInquirySent(true)
        setInquiryText('')
  ...
  const allPhotos = projects.flatMap(p =>
    p.recentPhotos.map(ph => ({ ...ph, projectName: p.name, projectId: p.id }))
  )
                  ...
                  <div className="p-5">
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between text-sm mb-1">
                        <span className="text-slate-600">進捗率</span>
                    ...
                    {project.recentPhotos.length > 0 && (
                      <div className="mt-4">
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-1">
                          {project.recentPhotos.slice(0, 5).map(photo => (
        ...
        {activeTab === 'photos' && (
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            ...
            {allPhotos.length === 0 ? (
              ...
              <div>
                {projects.filter(p => p.recentPhotos.length > 0).map(project => (
                  <div key={project.id} className="mb-6">
                    <p className="text-sm font-medium text-slate-700 mb-2">{project.name}</p>
